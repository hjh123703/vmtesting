...
RTN my_RTN_FindByName(IMG img, const CHAR* funcNameC_str)
{
	for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
	{
		for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
		{
			if (0 == RTN_Name(rtn).compare(funcNameC_str))
			{
				return rtn;
			}
		}
...
void HooksHandler::hookApiInThisLibrary(IMG img)
{
	...
	for (auto iter = libraryHooks.begin(); iter != libraryHooks.end(); ++iter)
	{
		...
		RTN rtn = my_RTN_FindByName(img, funcNameC_str);
...
BOOL isMsvcCpuid(unsigned char* toCheckBytes)
{
	...
	*/
	...
	unsigned char* const2 = new unsigned char[4] { 0x69, 0x6E, 0x65, 0x49 };
	unsigned char* const3 = new unsigned char[4] { 0x47, 0x65, 0x6E, 0x75 };
